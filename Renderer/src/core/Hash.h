...
constexpr static const u64 crc64table[256] = {
...
constexpr u64 crc64(std::string_view str)
{
	u64 crc = 0xffffffff;
	for (auto c : str)
		crc = (crc >> 8) ^ crc64table[(crc ^ c) & 0xff];
	return crc ^ 0xffffffff;
}

template<size_t N>
constexpr u64 crc64(StaticString<N> str)
{
...
constexpr u64 operator ""_hash(char const* s, size_t size)
{
 ...
 *
 */
inline uint32_t crc32(const u8 *buf, size_t size)
{
	const uint8_t *p = buf;
	uint32_t crc;
...
{
	return crc32(reinterpret_cast<u8 const*>(str.data()), str.size());
}
