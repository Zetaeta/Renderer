...
CompoundMeshData ObjReader::Parse(string file)
{
	...
	auto saveMesh = [&] {
		...
		Mesh& mesh = cMesh.meshes.emplace_back(AssetPath::File(file), std::move(vertices), std::move(faces), getName(), max(matId,0 ));
		matId = -1;
//		ComputeNormals(mesh);
		/* vertices.clear();
		vertIndsPlus1.clear();
...
MaterialID ObjReader::LoadMat(MtlMat& mat)
{
	...
	m_MatDB.emplace_back(std::make_shared<Material>(mat.name, vec4(mat.ambient, 1.f), glm::length(mat.specular), int(mat.specExp), glm::length(mat.diffuse), Texture::LoadFrom(("content/" + mat.diffuseMap).c_str())));
