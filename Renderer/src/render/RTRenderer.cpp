...
{
	vec3 start = m_Camera->GetPosition();
...
void RTRenderer::ComputeRayDirections()
{
	...
	for (u32 y=0; y<m_Height; ++y)
	{
		for (u32 x=0; x<m_Width; ++x)
		{
			m_RayDirections[x + y * m_Width] = m_Camera->GetView() * glm::normalize(vec3 { float(x) / m_PixelScale - xoffset, float(y) / m_PixelScale - yoffset, 1 });;
		}
	}
	m_Dirty = false;
}
