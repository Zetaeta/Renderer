...
void RastRenderer::Render(const Scene& scene)
{
	...
	for (auto const& mi : scene.m_MeshInstances)
	{
		...
		mat4			 model2World {trans};
		mat4 fullTrans = projWorld * model2World;
		for (auto tri : mesh.triangles)
		{
			...
			float const CLIP_X_MAX = 1;
			float const CLIP_X_MIN = -1;
			float const CLIP_Y_MAX = 1;
			float const CLIP_Y_MIN = -1;
			float const CLIP_Z_MAX = 1;
			float const CLIP_Z_MIN = 0;
			...
			ClipSpaceTri			   tri
			{
				ClipSpaceVertex {verts[0], TransVec(model2World, v0.normal), v0.uvs, TransPos(model2World, v0.pos)},
				ClipSpaceVertex {verts[1], TransVec(model2World, v1.normal), v1.uvs, TransPos(model2World, v1.pos)},
				ClipSpaceVertex {verts[2], TransVec(model2World, v2.normal), v2.uvs, TransPos(model2World, v2.pos)}
			};

			RecursiveClip<NUM_PLANES>(std::begin(clipPlanes), std::end(clipPlanes), tri, [&] (ClipSpaceTri const& clippedTri) {
				DrawTri(fullTrans, model2World,clippedTri, mesh.material);
			});
		}
	}
}
...
void RastRenderer::DrawLine(vec2 a, vec2 b)
{
	...
	if (abs(a.x - b.x) > abs(a.y - b.y))
	{
		...
		InterpolateToPix(a.x, a.y, b.x, b.y, 0, m_Width, [this](int x, int y) {
			if (x >= 0 && x < NumCast<int>(m_Width) && y >= 0 && y < NumCast<int>(m_Height))
			{
...
glm::u32 ClipTri(ClipSpaceTri const& tri, Plane const& plane, std::array<ClipSpaceTri, 2>& outClippedTris)
{
	using std::array;
	using std::begin;
	using std::end;
	...
	switch (wrongSide)
	{
		...
		case 1:
		{
			size_t	  out = find_if(begin(signedDist), end(signedDist), [](float sd) {
				  return sd < 0;
			  }) - signedDist.begin();
			size_t	  in1 = (out + 1) % 3, in2 = (out + 2) % 3;
		...
		case 2:
		{
			size_t	  in = find_if(begin(signedDist), end(signedDist), [](float sd) {
				  return sd >= 0;
			  }) - signedDist.begin();
			size_t	  out1 = (in + 1) % 3, out2 = (in + 2) % 3;
...
void RastRenderer::DrawTri(mat4 const& fullTrans, mat4 const& model2World, std::array<ClipSpaceVertex, 3> const& verts, MaterialID matId)
{
	vec2 const size = vec2 {m_Width, m_Height} / 2.f;
	vec2 const offcentre {1,1};
	auto toScreen = [size, offcentre] (vec2 ndc) {
		return (ndc + offcentre) * size;
	};
	std::array<vec2, 3> vs = { toScreen(vec2(verts[0].pos) / verts[0].pos.w), toScreen(vec2(verts[1].pos) / verts[1].pos.w), toScreen(vec2(verts[2].pos) / verts[2].pos.w) };
